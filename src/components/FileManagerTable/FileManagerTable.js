...
const folderIconUrl =
  "https://img.icons8.com/color/40/000000/folder-invoices--v2.png";
...
class FileManagerTable extends React.Component {
              ...
              res.json().then((data) => {
                ...
                for (var i = 1; i < path_arr.length; i++) {
                  ...
                  for (var j = 0; j < files.length; j++) {
                    ...
                    if (
                      path_arr[i] == files[j].name &&
                      files[j].type == "directory"
                    ) {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.currentFile !== this.props.currentFile) {
      this.state.responseObject.push({
        id:
          this.state.responseObject[this.state.responseObject.length - 1].id +
          1,
  ...
  renderTableHeader() {
    return (
      <tr className="header-row">
        ...
        <th colSpan="3" className="centriraj">
          File manipulation
        </th>
      </tr>
    );
    ...
    return this.state.responseObject.map((oneObject, index) => {
      ...
      return (
        <tr>
          ...
          <td
            className="file-download pomjereni-naziv file-icon"
            onClick={() => {
              this.handleClick(id);
            }}
          >
            <img
              className="file-icon"
              src={oneObject.data.type == "file" ? fileIconUrl : folderIconUrl}
            ></img>
            {fileName}
          </td>
          <div className="file-manipulation-wrapper">
            <td
              className="file-manipulation file-delete centriraj"
              onClick={() => {
            ...
            <td
              className="file-manipulation file-rename centriraj"
              onClick={() => {
            ...
            <td
              className="file-manipulation file-copy centriraj"
              onClick={() => {
                this.handleCopy(id);
              }}
            >
            ...
            <td
              className="file-manipulation file-move centriraj"
              onClick={() => {
                this.handleMove(id);
              }}
            >
            ...
            <Checkbox
              ...
              className="file-checkbox file-manipulation"
              color="default"
            />
          </div>
        </tr>
    ...
    try {
      ...
      if (response.status == 200) {
        ...
        var response1 = await fetch(
          "https://si-grupa5.herokuapp.com/api/web/user/file/delete",
          requestOptions2
        )
          .then((res) => {
            this.updateResponse();
  ...
  sendFolderDeleteRequest = async (name) => {
    try {
      ...
      if (response.status == 200) {
        ...
        var response1 = await fetch(
          "https://si-grupa5.herokuapp.com/api/web/user/folder/delete",
          requestOptions2
        )
          .then((res) => {
            this.updateResponse();
    ...
    if (file.data.type === "directory") {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          Swal.fire("Deleted!", "Your directory has been deleted.", "success");
        }
      });
    } else {
      Swal.fire({
  ...
  sendRenameRequest = async (oldFileName, newFileName, isDirectory) => {
    ...
    try {
      ...
      if (response.status == 200) {
        ...
        var response1 = await fetch(
          "https://si-grupa5.herokuapp.com/api/web/user/rename",
          requestOptions2
        )
          .then((res) => {
            Swal.fire({
  ...
  clickUp() {
    this.state.activeFolder = this.state.activeFolder
      .split("/")
      .slice(0, -1)
      .join("/");
    this.updateResponse();
  ...
  addFolder = async (newFolder) => {
    ...
    try {
      ...
      if (response.status == 200) {
        ...
        var response1 = await fetch(
          "https://si-grupa5.herokuapp.com/api/web/user/file/put",
          requestOptions2
        )
          .then((res) => {
            console.log(res.json());
  ...
  getText = async (file) => {
    ...
    try {
      ...
      if (response.status == 200) {
        ...
        return await fetch(
          "https://si-grupa5.herokuapp.com/api/web/user/file/get-text",
          requestOptions2
        )
          .then((res) => {
    ...
    return (
      <div>
        <button
          ...
          disabled={this.state.activeFolder == "." ? "disabled" : ""}
        >
          Go UP
        </button>
        ...
        <DragAndDrop
          updateView={this.updateResponse}
          activePath={this.state.activeFolder}
